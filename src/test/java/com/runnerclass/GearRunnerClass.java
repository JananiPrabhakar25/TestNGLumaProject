...
public class GearRunnerClass extends BaseClass_TestNG {
	...
	private void searchProductPage() {
		pageObject.getGearPageManager().getvalidgearsearch();
	}

	@Test(priority = 3)
	private void shippinPage() {
	...
	private void checkoutPage() {
		Assert.assertTrue(
				pageObject.getOrderCheckoutPageObjectManager().getValidCheckOutOrderPage(ExtentReport_Test.extenttest));
		staticWait(2000);
	}
